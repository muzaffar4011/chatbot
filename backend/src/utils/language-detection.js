...
export function detectLanguage(message) {
  ...
  // Also check if first word is a common Roman Urdu verb even if not in exact list
  if (firstWord.match(/^(batao|bata|batayein|batado|dekh|kar|karo|karein|karna|karne|karle)$/i)) {
    return 'ur';
  }
  ...
  // Step 5: Check for common Roman Urdu phrases at start or anywhere in first few words
  const urduPhrases = [
    ...
    'ke baare me', 'ke baare mein', 'ke bare me', 'ke bare mein', 'ka batao', 'ka bata', 'ki batao', 'ki bata',
    'ke baare', 'ke bare', 'baare me', 'baare mein', 'bare me', 'bare mein'
  ];
  
  // Check first 2-3 words for Urdu phrases
  const firstThreeWords = `${firstWord} ${secondWord} ${thirdWord}`.trim();
  for (const phrase of urduPhrases) {
    if (firstTwoWords.includes(phrase) || firstThreeWords.includes(phrase) || text.includes(phrase)) {
      return 'ur';
    }
  }

  // Step 6: Check for Roman Urdu postpositions and phrases (ke, ki, ka, me, mein, se, par, etc.)
  const urduPostpositions = /\b(ke|ki|ka|me|mein|se|par|tak|ko|ne|ka|ki|ke)\b/i;
  const urduCommonWords = /\b(batao|bata|batayein|batado|dekh|kar|karo|karein|ho|hai|hain|hen|hoga|hogi|hote|hota|hoti|mujhe|tumhe|hame|apke|apka|apki|mera|meri|mere|hamara|hamari|hamare|kya|kaun|kon|kahan|kaise|kyun|kab|kitna|kitne|kitni|baare|bare)\b/i;
  
  if (urduPostpositions.test(text) || urduCommonWords.test(text)) {
    // If we find Urdu postpositions or common words, check if there are more Urdu indicators
    const urduIndicators = (text.match(urduPostpositions) || []).length + (text.match(urduCommonWords) || []).length;
    if (urduIndicators >= 2) {
      return 'ur';
    }
  }
  
  // Step 7: Count Urdu vs English words in first 3 words
  let urduCount = 0;
  let englishCount = 0;
  
  for (const word of words) {
    if (romanUrduExactWords.has(word) || word.match(/^(kya|kaun|kon|apke|apka|mera|mujhe|hamara|batao|kar|ho|hai|hain|hen|ke|ki|ka|me|mein|se|par|baare|bare)/i)) {
      urduCount++;
    }
  ...
  // Also check entire text for Urdu indicators
  if (text.match(/\b(ke baare|ke bare|baare me|baare mein|batao|bata|batayein)\b/i)) {
    urduCount += 2; // Strong indicator
  }
